...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login({ name: data.user.name, role: data.user.role, token: data.token });
      router.push("/pos");
    } catch (err: any) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setSignupError("");
    try {
      const data = await apiClient.post("/auth/signup", { name: signupName, email: signupEmail, password: signupPassword });
      login({ name: data.user.name, role: data.user.role, token: data.token });
      router.push("/pos");
    } catch (err: any) {
  ...
  return (
    <div className="flex min-h-screen bg-cafe-bg items-center justify-center p-8 gap-8 flex-col lg:flex-row">
      {/* Login Card */}
      <div className="w-full max-w-md bg-cafe-card rounded-xl shadow-lg p-8">
